...
async function main() {
  ...
  const layout = stack()
    ...
    .add(
      stack()
        .margin({ left: 200 })
        .text('Indented 200 from left')
    )
    .text('margin({ right: 200 }) - pushes from right:')
    .add(
      stack()
        .margin({ right: 200 })
        .text('200 margin on right')
    )
    .spacer(10)
