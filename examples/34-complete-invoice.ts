...
async function main() {
  ...
  engine.setData({
    ...
    items: [
      { name: 'Wireless Mouse Pro', quantity: 10, unitPrice: 39.99, total: 399.90 },
      { name: 'Mechanical Keyboard', quantity: 5, unitPrice: 89.99, total: 449.95 },
      { name: 'USB-C Hub (7-port)', quantity: 10, unitPrice: 49.99, total: 499.90 },
      { name: 'Monitor Stand 27"', quantity: 5, unitPrice: 79.99, total: 399.95 },
      { name: 'Webcam HD 1080p', quantity: 10, unitPrice: 59.99, total: 599.90 },
    ],
    totals: {
      subtotal: 2349.60,
  ...
  const invoice = stack()
      ...
      flex()
        ...
        .add(
          conditional()
            ...
            .elseIf(eq('customer.membership', 'platinum'), text('*** PLATINUM VIP ***', { bold: true }))
            .else(spacer(0))
            .build()
        )
    )
    ...
    // === TOTALS ===
    .add(flex().spacer().text('Subtotal:').add(template('{{totals.subtotal | currency:"$"}}').width(120).align('right').build()))
    .add(
      conditional()
        ...
        .then(
          flex()
            ...
            .add(template('-{{totals.discount.amount | currency:"$"}}').width(120).align('right').italic().build())
        )
        .else(spacer(0))
        .build()
    )
      ...
      flex()
        ...
        .add(
          conditional()
            ...
            .then(template('{{totals.shipping.cost | currency:"$"}}').width(120).align('right').build())
            .else(text('FREE', { width: 120, align: 'right', bold: true }))
            .build()
        )
    )
    .add(flex().spacer().add(template('Tax ({{totals.tax.rate}}%):').build()).add(template('{{totals.tax.amount | currency:"$"}}').width(120).align('right').build()))
    .add(flex().spacer().add(line('-', 200)))
    .add(flex().spacer().text('TOTAL DUE:', { bold: true }).add(template('{{totals.grandTotal | currency:"$"}}').width(120).align('right').bold().build()))
      ...
      flex()
        ...
        .add(
          switchOn('payment.method')
            ...
            .case('credit_card', template('Card ****{{payment.lastFour}}'))
            .default(text('Other'))
            .build()
        )
        .spacer()
