  ...
  for (const record of records) {
    switch (record.type) {
      case 'ESC_@':
        ...
        break;
      case 'ESC_J':
        console.log(`  [${record.byteOffset.toString().padStart(4)}] ESC J : Y += ${record.dots} dots (${record.value}/180")`);
        console.log(`         Hex: ${record.hexBytes}`);
        console.log(`         Y was ${currentY}, now ${currentY + (record.dots ?? 0)}`);
        currentY += record.dots ?? 0;
        break;
      case 'ESC_$':
        console.log(`  [${record.byteOffset.toString().padStart(4)}] ESC $ : X = ${record.dots} dots (${record.value} units)`);
        console.log(`         Hex: ${record.hexBytes}`);
        currentX = record.dots ?? 0;
        break;
      case 'TEXT':
        const width = (record.text?.length ?? 0) * 36;
        console.log(`  [${record.byteOffset.toString().padStart(4)}] TEXT  : "${record.text}" at (${currentX}, ${currentY})`);
        currentX += width;
        break;
    }
  }
...
async function main(): Promise<void> {
  ...
  await renderPreview(prn1, 'Test 1: Stack gap=0', 'qa-prn-stack-01-no-gap', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn2, 'Test 2: Stack gap=60', 'qa-prn-stack-02-gap-60', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn3, 'Test 3: Stack gap=120', 'qa-prn-stack-03-gap-120', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn4, 'Test 4: Stack direction=row', 'qa-prn-stack-04-row', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn5, 'Test 5: Nested stacks', 'qa-prn-stack-05-nested', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn6, 'Test 6: Stack align=center', 'qa-prn-stack-06-centered', { paper: ZERO_MARGIN_PAPER });
