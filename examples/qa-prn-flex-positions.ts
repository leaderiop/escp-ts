  ...
  while (i < data.length) {
    ...
    if (byte === 0x1B) { // ESC
      ...
      if (nextByte === 0x24) { // ESC $
        ...
        if (nextByte === 0x45 || nextByte === 0x46) i += 0; // Bold on/off
        else if (nextByte === 0x57) i += 1; // Double width
      ...
      records.push({
        ...
        hexBytes: text.split('').map(c => c.charCodeAt(0).toString(16).padStart(2, '0').toUpperCase()).join(' '),
        textContent: text,
      });
    } else {
      i++;
...
function analyzePositioning(label: string, data: Uint8Array): void {
  ...
  for (const record of records) {
    if (record.type === 'ESC_$') {
      console.log(`  [${record.byteOffset.toString().padStart(4)}] ESC $ : X = ${record.dotsCalculated} dots (was ${currentX})`);
      ...
      console.log(`  [${record.byteOffset.toString().padStart(4)}] ESC J : Y += ${record.dotsCalculated} dots (Y now ${currentY + (record.dotsCalculated ?? 0)})`);
      console.log(`         Hex: ${record.hexBytes}`);
      ...
      console.log(`  [${record.byteOffset.toString().padStart(4)}] TEXT  : "${record.textContent}" at X=${currentX}`);
      console.log(`         Width: ${record.textContent?.length} chars * ${charWidth} = ${textWidth} dots`);
      currentX += textWidth;
    } else if (record.type === 'ESC_@') {
      console.log(`  [${record.byteOffset.toString().padStart(4)}] ESC @ : Initialize printer (reset)`);
      currentX = 0;
      currentY = 0;
    }
  }
...
async function main(): Promise<void> {
  ...
  await renderPreview(prn1, 'Test 1: Flex gap=0', 'qa-prn-flex-01-no-gap', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn2, 'Test 2: Flex gap=36', 'qa-prn-flex-02-gap-36', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn3, 'Test 3: Flex justify=end', 'qa-prn-flex-03-justify-end', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn4, 'Test 4: Flex justify=center', 'qa-prn-flex-04-justify-center', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn5, 'Test 5: Flex justify=space-between', 'qa-prn-flex-05-space-between', { paper: ZERO_MARGIN_PAPER });
  ...
  await renderPreview(prn6, 'Test 6: Flex width=500', 'qa-prn-flex-06-width-constraint', { paper: ZERO_MARGIN_PAPER });
