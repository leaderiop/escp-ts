...
async function main() {
  ...
  const testCases: PositionTest[] = [
    ...
    { name: '256 units', xDots: 1536, yDots: 120, expectedEscDollar: calculateExpectedPosition(1536) },

    // Test at 300 units (1800 dots)
    { name: '300 units', xDots: 1800, yDots: 120, expectedEscDollar: calculateExpectedPosition(1800) },

    // Test at 512 units (3072 dots)
    { name: '512 units', xDots: 3072, yDots: 180, expectedEscDollar: calculateExpectedPosition(3072) },

    // Test at 600 units (3600 dots - 10 inches)
    { name: '600 units', xDots: 3600, yDots: 180, expectedEscDollar: calculateExpectedPosition(3600) },
  ];
  ...
  testCases.forEach((test) => {
    layoutBuilder = layoutBuilder.add(
      stack()
        .absolutePosition(test.xDots, test.yDots)
        .text(`[${test.name}]`)
    );
  });
  ...
  testCases.forEach((test) => {
    ...
    console.log(`  Expected ESC $: nL=0x${expected.nL.toString(16).padStart(2, '0')} nH=0x${expected.nH.toString(16).padStart(2, '0')} (${expected.nL + expected.nH * 256} units)`);
  });

  // Parse actual ESC $ commands from output
  console.log('\n=== ACTUAL ESC $ COMMANDS IN OUTPUT ===\n');
  const escDollarCommands: Array<{ offset: number; nL: number; nH: number; units: number; dots: number }> = [];
  ...
  for (let i = 0; i < commands.length - 3; i++) {
    if (commands[i] === 0x1B && commands[i + 1] === 0x24) {
      ...
      console.log(`Offset 0x${i.toString(16).padStart(4, '0')}: ESC $ 0x${nL.toString(16).padStart(2, '0')} 0x${nH.toString(16).padStart(2, '0')} = ${units} units = ${dots} dots = ${dots / 360} inches`);
    }
  }
  ...
  for (let i = 0; i < commands.length - 2; i++) {
    if (commands[i] === 0x1B && commands[i + 1] === 0x4A) {
      ...
      console.log(`Offset 0x${i.toString(16).padStart(4, '0')}: ESC J 0x${n.toString(16).padStart(2, '0')} = ${n}/180 inch = ${dots} dots = ${dots / 360} inches`);
    }
  }
  ...
  testCases.forEach((test) => {
    ...
    if (found) {
      console.log(`[PASS] ${test.name}: Found ESC $ with ${found.units} units (expected ${expectedUnits})`);
    } else {
