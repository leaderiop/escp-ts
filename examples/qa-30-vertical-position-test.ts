...
async function main() {
  ...
  testCases.forEach((test, idx) => {
    layoutBuilder = layoutBuilder.add(
      stack()
        .absolutePosition(100, test.yDots)
        .text(`Y${idx}`)
    );
  });
  ...
  escJCommands.forEach((cmd, idx) => {
    console.log(`ESC J[${idx}] at 0x${cmd.offset.toString(16).padStart(4, '0')}: n=${cmd.n} (0x${cmd.n.toString(16).padStart(2, '0')}) = ${cmd.dots} dots = ${(cmd.dots / 360).toFixed(4)} inches`);
  });
  ...
  for (let i = 0; i < commands.length; i++) {
    if (commands[i] === 0x1B && commands[i + 1] === 0x4A) {
      const n = commands[i + 2];
      currentY += n * 2;
      console.log(`After ESC J ${n}: currentY = ${currentY} dots = ${(currentY / 360).toFixed(4)} inches`);
      i += 2;
    } else if (commands[i] === 0x59 && i > 0 && commands[i - 1] >= 0x30 && commands[i - 1] <= 0x39) {
