...
async function main() {
  ...
  const layout = stack()
    ...
    // Title
    .text('INTERNATIONAL CHARACTER SETS', { bold: true, doubleWidth: true, align: 'center' })
    .line('=', 'fill')
    .spacer(20)
  ...
  // Define all charsets
  const charsets: Array<{ name: string; value: number; sample: string }> = [
    { name: 'USA', value: INTERNATIONAL_CHARSET.USA, sample: 'Hello, World! Price: $99.99' },
    { name: 'France', value: INTERNATIONAL_CHARSET.FRANCE, sample: 'Bonjour! Prix: 99,99 EUR' },
    { name: 'Germany', value: INTERNATIONAL_CHARSET.GERMANY, sample: 'Guten Tag! Preis: 99,99 EUR' },
    { name: 'United Kingdom', value: INTERNATIONAL_CHARSET.UK, sample: 'Hello! Price: 99.99 GBP' },
    { name: 'Denmark I', value: INTERNATIONAL_CHARSET.DENMARK_I, sample: 'Hej! Pris: 99,99 DKK' },
    { name: 'Sweden', value: INTERNATIONAL_CHARSET.SWEDEN, sample: 'Hej! Pris: 99,99 SEK' },
    { name: 'Italy', value: INTERNATIONAL_CHARSET.ITALY, sample: 'Ciao! Prezzo: 99,99 EUR' },
    { name: 'Spain I', value: INTERNATIONAL_CHARSET.SPAIN_I, sample: 'Hola! Precio: 99,99 EUR' },
    { name: 'Japan', value: INTERNATIONAL_CHARSET.JAPAN, sample: 'Price: 9999 JPY' },
    { name: 'Norway', value: INTERNATIONAL_CHARSET.NORWAY, sample: 'Hei! Pris: 99,99 NOK' },
    { name: 'Denmark II', value: INTERNATIONAL_CHARSET.DENMARK_II, sample: 'Hej! Pris: 99,99 DKK' },
    { name: 'Spain II', value: INTERNATIONAL_CHARSET.SPAIN_II, sample: 'Hola! Precio: 99,99 EUR' },
    { name: 'Latin America', value: INTERNATIONAL_CHARSET.LATIN_AMERICA, sample: 'Hola! Precio: $99.99' },
  ];
  ...
  for (const cs of charsets) {
    ...
    engine.setBold(true).print(`${cs.name.padEnd(18)}: `).setBold(false);
  ...
  for (const cs of charsets.slice(0, 8)) { // First 8 for space
    engine.setInternationalCharset(cs.value);
