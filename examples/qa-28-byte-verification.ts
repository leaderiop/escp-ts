...
import { LayoutEngine, stack, text } from '../src/index';
  ...
  while (i < commands.length) {
    if (commands[i] === 0x1B) {
      ...
      if (cmd === 0x40) {
        ...
        const pos = nL + nH * 256;
        const dots = pos * 6;
        console.log(`[${i.toString(16).padStart(4, '0')}] ESC $ ${nL.toString(16).padStart(2, '0')} ${nH.toString(16).padStart(2, '0')} - Horizontal position: ${pos} units = ${dots} dots = ${(dots/360).toFixed(3)} inches`);
        i += 4;
      } else if (cmd === 0x4A) {
        const n = commands[i + 2];
        const dots = n * 2;
        console.log(`[${i.toString(16).padStart(4, '0')}] ESC J ${n.toString(16).padStart(2, '0')} - Vertical advance: ${n}/180 inch = ${dots} dots = ${(dots/360).toFixed(3)} inches`);
        i += 3;
      } else if (cmd === 0x45) {
        ...
        const n = commands[i + 2];
        console.log(`[${i.toString(16).padStart(4, '0')}] ESC W ${n} - Double width ${n ? 'on' : 'off'}`);
        i += 3;
      } else if (cmd === 0x34) {
        ...
        console.log(`[${i.toString(16).padStart(4, '0')}] ESC ${cmd.toString(16).padStart(2, '0')} (${String.fromCharCode(cmd)})`);
        i += 2;
      }
      ...
      if (text.length <= 40) {
        ...
        console.log(`[${startPos.toString(16).padStart(4, '0')}] TEXT: "${text.slice(0, 37)}..." (${text.length} chars)`);
      }
    } else {
      i++;
    }
