...
import { LayoutEngine, gt, exists, empty } from '@escp/jsx';
import { Stack, Flex, Text, Line, Spacer, Template, If, Switch, Case } from '@escp/jsx';
import type { LayoutNode, DataContext } from '@escp/jsx';
  ...
  const layout = (
    <Stack style={{ gap: 10, padding: 30 }}>
      ...
      <Text style={{ italic: true }}>
        {'Using <Switch path="..."><Case value="...">...</Case></Switch>'}
      </Text>
      <Spacer style={{ height: 8 }} />

      {/* Order status */}
      <Switch path="order.status" default={(<Text>[?] Unknown Status</Text>) as LayoutNode}>
        <Case value="pending">
      ...
      <If
        condition={gt('order.total', 200)}
          ...
          (
            <Template template={'Add {{order.total | currency:"$"}} more for free shipping'} />
          ) as LayoutNode
        }
      >
      ...
      <Spacer style={{ height: 8 }} />
      <Switch
        path="user.membershipTier"
        default={(<Text>Tier: Bronze - Welcome!</Text>) as LayoutNode}
      >
        <Case value="platinum">
          <Flex>
            <Text>Tier: </Text>
      ...
      <Switch
        path="user.membershipTier"
        default={(<Template template="Welcome, {{user.name}}!" />) as LayoutNode}
      >
        <Case value="platinum">
      ...
      <Text style={{ italic: true }} align="center">
        End of Data-Driven Conditionals Demo
      </Text>
    </Stack>
  ) as LayoutNode;
