...
async function main() {
  ...
  const layout = stack()
    ...
    .add(
      flex()
        .gap(50)
        .add(stack().text('Horizontal:', { bold: true }).text('HELLO'))
        .add(stack().text('Vertical:', { bold: true }).text('HELLO', { orientation: 'vertical' }))
    )
    .spacer(30)
    ...
    .add(
      flex()
        .gap(20)
        .add(text('LABEL', { orientation: 'vertical', bold: true }))
        .add(
          stack()
    ...
    .add(
      flex()
        .gap(100)
        .add(stack().text('TEST', { bold: true }).text('(horizontal)', { italic: true }))
        .add(
          stack()
