...
function analyzeAbsolutePositioning(
  label: string,
  data: Uint8Array,
  expectedX: number,
  expectedY: number
  ...
  for (const cmd of commands) {
    switch (cmd.type) {
      ...
      case 'ESC_(V':
        console.log(
          `  [${cmd.offset.toString().padStart(4)}] ESC (V: Y = ${cmd.yDots} dots (absolute)`
        );
        currentY = cmd.yDots ?? 0;
        break;
      case 'ESC_(v':
        console.log(
          `  [${cmd.offset.toString().padStart(4)}] ESC (v: Y += ${cmd.yDots} dots (relative)`
        );
        currentY += cmd.yDots ?? 0;
        break;
      case 'TEXT':
        console.log(
          `  [${cmd.offset.toString().padStart(4)}] TEXT  : "${cmd.text}" at (${currentX}, ${currentY})`
        );
...
async function main(): Promise<void> {
  ...
  engine2.initialize();
  engine2.render(stack().absolutePosition(100, 0).text('X=100').build());
  ...
  engine3.initialize();
  engine3.render(stack().absolutePosition(0, 100).text('Y=100').build());
  ...
  engine4.initialize();
  engine4.render(stack().absolutePosition(200, 150).text('HERE').build());
  ...
  await renderPreview(prn4, 'Test 4: XY Position', 'qa-prn-abs-04-xy', {
  ...
  await renderPreview(prn5, 'Test 5: Multiple', 'qa-prn-abs-05-multi', {
    paper: ZERO_MARGIN_PAPER,
  });
  ...
  await renderPreview(prn6, 'Test 6: Relative', 'qa-prn-abs-06-relative', {
    paper: ZERO_MARGIN_PAPER,
  });
  ...
  engine7.initialize();
  engine7.render(stack().absolutePosition(0, 600).text('Far down').build());
  ...
  await renderPreview(prn7, 'Test 7: Large Y', 'qa-prn-abs-07-large-y', {
    paper: ZERO_MARGIN_PAPER,
  });
