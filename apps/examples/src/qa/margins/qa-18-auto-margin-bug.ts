...
async function main() {
  ...
  const layout = stack()
    ...
    .add(stack().margin('auto').width(600).padding(10).text('Short text', { align: 'center' }))
    .spacer(15)

    // Comparison: Different widths centered
    .text('WIDTH COMPARISON (all centered):', { bold: true, underline: true })
    .add(stack().margin('auto').width(400).padding(5).text('Width: 400', { align: 'center' }))
    .add(stack().margin('auto').width(600).padding(5).text('Width: 600', { align: 'center' }))
    .add(stack().margin('auto').width(800).padding(5).text('Width: 800', { align: 'center' }))
    .add(stack().margin('auto').width(1000).padding(5).text('Width: 1000', { align: 'center' }))
    .spacer(15)
