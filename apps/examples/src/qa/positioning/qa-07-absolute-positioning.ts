...
async function main() {
  ...
  const layout = stack()
    ...
    .add(stack().absolutePosition(0, 0).text('[AT ORIGIN 0,0]', { bold: true }))
    .spacer(30)
    ...
    .add(stack().absolutePosition(50, 400).text('[X=50]'))
    .add(stack().absolutePosition(300, 400).text('[X=300]'))
    .add(stack().absolutePosition(550, 400).text('[X=550]'))
    .spacer(30)

    // Overlapping elements
    .text('OVERLAPPING ELEMENTS', { bold: true, underline: true })
    .text('Three elements at nearly same position (500, 500):')
    .add(stack().absolutePosition(500, 500).text('Layer 1 -----'))
    .add(stack().absolutePosition(510, 520).text('Layer 2 -----'))
    .add(stack().absolutePosition(520, 540).text('Layer 3 -----'))
    .spacer(80)
    ...
    .add(stack().absolutePosition(600, 700).text('<<< ABSOLUTE AT (600,700)'))
    ...
    .add(stack().absolutePosition(4500, 1300).text('[RIGHT EDGE]'))
    .spacer(30)
