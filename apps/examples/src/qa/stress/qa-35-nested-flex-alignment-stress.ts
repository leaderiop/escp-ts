...
async function main() {
  ...
  const layout = stack()
    ...
    .text('TEST 1: Triple nested flex - justify: start > center > end', {
      bold: true,
      underline: true,
    })
    .add(
      flex()
        ...
        .add(
          flex()
            ...
            .width(600)
            .gap(5)
            .add(flex().justify('end').width(500).gap(5).text('[Inner-1]').text('[Inner-2]'))
            .text('[Mid-1]')
        )
        .text('[Outer-1]')
    )
    ...
    .text('TEST 2: Nested flex - alignItems: top > center > bottom', {
      bold: true,
      underline: true,
    })
    .add(
      flex()
        .alignItems('top')
        .gap(20)
        .add(stack().width(150).padding(30).text('[Tall-Top]'))
        .add(
          flex()
            .alignItems('center')
            .width(500)
        ...
        .add(stack().width(150).padding(10).text('[Short-Top]'))
    )
    .spacer(15)
      ...
      flex()
        ...
        .add(
          stack()
            ...
            .add(flex().justify('end').gap(10).text('[Nested-X]').text('[Nested-Y]'))
            .text('Stack footer 2')
        )
    )
    .spacer(15)
    ...
    .add(
      flex()
        .justify('start')
        .gap(20)
        .add(flex().width('30%').justify('center').gap(5).text('[30%-A]').text('[30%-B]'))
        .add(
          flex()
            .width(800)
            .justify('space-between')
        ...
        .add(flex().width('20%').justify('end').gap(5).text('[20%]'))
    )
    .spacer(15)
      ...
      flex()
        ...
        .add(
          flex()
            .justify('end')
            .width(800)
            .add(flex().justify('start').width(400).text('[Single deeply nested]'))
        )
    )
    .spacer(15)
  ...
  await renderPreview(
    commands,
    'QA: Nested Flex Alignment Stress',
    'qa-35-nested-flex-alignment-stress'
  );
}
