...
async function runExample(example: (typeof examples)[0]): Promise<Result> {
  const start = Date.now();
  const filePath = path.resolve(rootDir, 'src', example.path);
...
async function main() {
  ...
  for (const example of examples) {
    process.stdout.write(`[${example.id}] ${example.name}... `);
