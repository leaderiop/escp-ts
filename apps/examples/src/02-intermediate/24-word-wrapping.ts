...
async function main() {
  ...
  const mediumText =
    'This is a medium-length paragraph that demonstrates how text is wrapped at word boundaries when it exceeds the printable width of the page.';
  const longText =
  ...
  const demoText =
    'The quick brown fox jumps over the lazy dog. This sentence is used to demonstrate how different character densities affect word wrapping behavior.';
  ...
  const styledText =
    'This text demonstrates how word wrapping adjusts for different font styles and widths.';
  ...
  const paragraph1 =
    'This is the first paragraph. It contains introductory information about the topic at hand. Notice how each paragraph is separated by blank lines for readability.';
  const paragraph2 =
    'The second paragraph continues the discussion. Word wrapping ensures that text flows naturally from one line to the next without breaking words inappropriately.';
  const paragraph3 =
    'Finally, the third paragraph concludes our example. Automatic word wrapping is essential for printing documents, reports, and any text that exceeds a single line.';
