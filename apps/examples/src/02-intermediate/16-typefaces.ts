...
import { LayoutEngine, TYPEFACE, PRINT_QUALITY, stack, flex, line, type Typeface } from '@escp/jsx';
...
async function main() {
  ...
  const typefaces: Array<{ name: string; value: Typeface }> = [
  ...
  for (const tf of typefaces.slice(0, 6)) {
    // First 6 typefaces
    engine
      .setTypeface(tf.value)
