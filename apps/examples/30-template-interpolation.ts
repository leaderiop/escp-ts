...
import { LayoutEngine, stack, flex, template, line } from '@escp/jsx';
...
async function main() {
  ...
  const layout = stack()
    ...
    .add(
      template(
        'City: {{company.address.city}}, {{company.address.state}} {{company.address.zip}}'
      ).build()
    )
    .spacer(15)
    ...
    .spacer(8)
    .add(flex().text('Original:').add(template('{{greeting}}').build()))
    .add(flex().text('Trimmed:').add(template('{{greeting | trim}}').build()))
    .add(flex().text('Uppercase:').add(template('{{customer.name | uppercase}}').build()))
    .add(flex().text('Lowercase:').add(template('{{customer.email | lowercase}}').build()))
    .add(flex().text('Capitalized:').add(template('{{company.tagline | capitalize}}').build()))
    .add(flex().text('Truncated:').add(template('{{company.tagline | truncate:20}}').build()))
    .spacer(15)
    ...
    .spacer(8)
    .add(flex().text('Order Total:').add(template('{{order.total | currency:"$"}}').build()))
    .add(flex().text('Discount:').add(template('{{order.discount | currency:"$"}}').build()))
    .add(flex().text('Tax:').add(template('{{order.tax | currency:"$"}}').build()))
    .add(
      flex().text('Loyalty Points:').add(template('{{customer.loyaltyPoints | number}}').build())
    )
    .spacer(15)
    ...
    .spacer(8)
    .add(flex().text('Phone:').add(template('{{customer.phone | default:"Not provided"}}').build()))
    .add(flex().text('Notes:').add(template('{{order.notes | default:"None"}}').build()))
    .spacer(15)
