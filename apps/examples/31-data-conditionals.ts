  ...
  const layout = stack()
    ...
    .add(
      conditional()
        ...
        .elseIf(
          eq('user.membershipTier', 'gold'),
          text('Tier: GOLD - Excellent status!', { bold: true })
        )
    ...
    .add(flex().text('Items:').spacer().add(template('{{order.itemCount}}').build()))
    .add(
      conditional()
        .ifPath('order.shippingMethod', 'eq', 'express')
        .then(flex().text('Shipping:').spacer().text('EXPRESS (1-2 days)', { bold: true }))
        .else(flex().text('Shipping:').spacer().text('Standard (5-7 days)'))
        .build()
    )
    .spacer(5)
    .line('-', 'fill')
