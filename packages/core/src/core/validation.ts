 ...
 */
export function assertRange(value: number, min: number, max: number, parameter: string): void {
...
export function assertValidHex(hex: string): void {
  ...
  if (clean.length % 2 !== 0) {
    throw new EncodingError(`Hex string must have even length, got ${clean.length}`, hex);
  }
 ...
 */
export function assertPositiveDimensions(width: number, height: number, operation: string): void {
  if (!Number.isInteger(width) || width < 1) {
    throw new GraphicsError(`Width must be a positive integer, got ${width}`, operation);
  }
  if (!Number.isInteger(height) || height < 1) {
    throw new GraphicsError(`Height must be a positive integer, got ${height}`, operation);
  }
}
...
export function assertNonNegative(value: number, parameter: string): void {
  if (value < 0) {
    throw new ValidationError(`${parameter} cannot be negative, got ${value}`, parameter, value);
  }
}
 ...
 */
export function assertOneOf<T>(value: T, allowed: readonly T[], parameter: string): void {
