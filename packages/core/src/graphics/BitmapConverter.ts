...
export const GRAPHICS_MODES: Record<BitImageMode, GraphicsModeInfo> = {
  // 8-pin modes
  [BIT_IMAGE_MODE.SINGLE_DENSITY_8PIN]: {
    pins: 8,
    horizontalDpi: 60,
    bytesPerColumn: 1,
    adjacentDots: true,
  },
  [BIT_IMAGE_MODE.DOUBLE_DENSITY_8PIN]: {
    pins: 8,
    horizontalDpi: 120,
    bytesPerColumn: 1,
    adjacentDots: false,
  },
  [BIT_IMAGE_MODE.HIGH_SPEED_DOUBLE_8PIN]: {
    pins: 8,
    horizontalDpi: 120,
    bytesPerColumn: 1,
    adjacentDots: true,
  },
  [BIT_IMAGE_MODE.QUAD_DENSITY_8PIN]: {
    pins: 8,
    horizontalDpi: 240,
    bytesPerColumn: 1,
    adjacentDots: false,
  },
  [BIT_IMAGE_MODE.CRT_I_8PIN]: {
    pins: 8,
    horizontalDpi: 72,
    bytesPerColumn: 1,
    adjacentDots: true,
  },
  [BIT_IMAGE_MODE.CRT_II_8PIN]: {
    pins: 8,
    horizontalDpi: 90,
    bytesPerColumn: 1,
    adjacentDots: true,
  },
  [BIT_IMAGE_MODE.PLOTTER_8PIN]: {
    pins: 8,
    horizontalDpi: 80,
    bytesPerColumn: 1,
    adjacentDots: true,
  ...
  // 24-pin modes
  [BIT_IMAGE_MODE.SINGLE_DENSITY_24PIN]: {
    pins: 24,
    horizontalDpi: 60,
    bytesPerColumn: 3,
    adjacentDots: true,
  },
  [BIT_IMAGE_MODE.DOUBLE_DENSITY_24PIN]: {
    pins: 24,
    horizontalDpi: 120,
    bytesPerColumn: 3,
    adjacentDots: false,
  },
  [BIT_IMAGE_MODE.HIGH_SPEED_DOUBLE_24PIN]: {
    pins: 24,
    horizontalDpi: 120,
    bytesPerColumn: 3,
    adjacentDots: true,
  },
  [BIT_IMAGE_MODE.TRIPLE_DENSITY_24PIN]: {
    pins: 24,
    horizontalDpi: 180,
    bytesPerColumn: 3,
    adjacentDots: true,
  },
  [BIT_IMAGE_MODE.HEX_DENSITY_24PIN]: {
    pins: 24,
    horizontalDpi: 360,
    bytesPerColumn: 3,
    adjacentDots: true,
  },
  [BIT_IMAGE_MODE.CRT_III_24PIN]: {
    pins: 24,
    horizontalDpi: 90,
    bytesPerColumn: 3,
    adjacentDots: true,
  ...
  for (let y = 0; y < image.height; y++) {
    for (let x = 0; x < image.width; x++) {
      ...
      if (y + 1 < image.height) {
        ...
        if (x + 1 < image.width)
          errors[idx + image.width + 1] = (errors[idx + image.width + 1] ?? 0) + error;
      }
      if (y + 2 < image.height) {
...
export function scaleImageBilinear(
  ...
  for (let y = 0; y < newHeight; y++) {
    for (let x = 0; x < newWidth; x++) {
      ...
      const value =
        p11 * (1 - fx) * (1 - fy) + p12 * fx * (1 - fy) + p21 * (1 - fx) * fy + p22 * fx * fy;
 ...
 */
export function createCheckerboard(width: number, height: number, cellSize = 8): GrayscaleImage {
  const data = new Uint8Array(width * height);
