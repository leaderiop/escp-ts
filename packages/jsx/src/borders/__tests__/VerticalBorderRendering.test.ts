...
import { getGridBorderCharSet, calculateTextWidth } from '@escp/core';
...
describe('Vertical Border Rendering', () => {
  ...
  describe('Border Character Layout', () => {
    ...
    it('should have border chars with width >= character intrinsic width', () => {
      const chars = getGridBorderCharSet('single');
      const cells = [Stack({ style: { flexGrow: 1 }, children: Text({ children: 'A' }) })];
      const row = wrapCellsWithVerticalBorders(cells, chars) as LayoutNode;
  ...
  describe('Horizontal Border Rows', () => {
    ...
    it('should have junction chars in row separator', () => {
      ...
      const leftJunction = (children[0] as { children: LayoutNode[] }).children[0] as {
        content: string;
      };
      const middleJunction = (children[2] as { children: LayoutNode[] }).children[0] as {
        content: string;
      };
      const rightJunction = (children[4] as { children: LayoutNode[] }).children[0] as {
        content: string;
      };
