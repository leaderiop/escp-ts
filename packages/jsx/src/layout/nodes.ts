...
import {
  resolveTypefaceValue,
  resolvePrintQualityValue,
  type TypefaceValue,
 ...
 */
export type ContentCondition = ((ctx: SpaceContext) => boolean) | SpaceQuery;
 ...
 */
export function isResolvableNode(
  node: LayoutNode
): node is TemplateNode | ConditionalNode | SwitchNode | EachNode {
  return (
    node.type === 'template' ||
...
export function resolveStyle(node: StyleProps, parentStyle: ResolvedStyle): ResolvedStyle {
  return {
    ...
    typeface:
      node.typeface !== undefined ? resolveTypefaceValue(node.typeface) : parentStyle.typeface,
    printQuality:
      node.printQuality !== undefined
